...
open class KGDrawerViewController: UIViewController {
    ...
    // MARK: - Gesture
    func prepareGesture() {
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(KGDrawerViewController.handlerGesture(_:)))
        panGesture.maximumNumberOfTouches = 1
        drawerView.centerViewContainer.addGestureRecognizer(panGesture)
    }
    ...
    func handlerGesture(_ sender: UIPanGestureRecognizer) {
        let translation = sender.translation(in: sender.view?.superview!)
        var progress = translation.x / drawerView.leftViewContainerWidth * (isOpening ? 1.0 : -1.0)
        progress = min(max(progress, 0.0), 1.0)
        print(translation)
        switch sender.state {
        case .began:
            let isOpen = floor(drawerView.centerViewContainer.frame.origin.x / drawerView.leftViewContainerWidth)
    ...
    func setMenu(toPercent percent: CGFloat) {
        drawerView.centerViewContainer.frame.origin.x = drawerView.leftViewContainerWidth * CGFloat(percent)
    }
}
